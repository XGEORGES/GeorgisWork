...
/**
 * Renderiza la interfaz de la Pantalla 5 (Reporte de Horas Extras - Estilo Syntrix)
 */
    ...
    <div class="space-y-6 pb-16">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Cálculo laboral según jornada ordinaria (L-V 8:00-17:30, Sáb 8:00-13:30, Dom/Feriados 100%) y reglas de redondeo.</p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Total Horas Extras (Redondeadas)</div>
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Registros con Sobretiempo</div>
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Horas Extras Exactas (Sin Redondeo)</div>
            <div id="kpi-horas-exactas" class="text-2xl font-mono font-bold text-cyan-600 dark:text-cyan-400 mt-0.5">0.00 h</div>
          </div>
        </div>
      ...
      <div class="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white/90 dark:bg-[#111827]/75 shadow-md">
        
        <div class="flex flex-wrap items-center gap-3">
          <div class="flex items-center space-x-2">
            <label for="filtro-he-desde" class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Fecha Inicial:</label>
            <input 
              type="date" 
              id="filtro-he-desde" 
              value="${fechaInicioFiltro}" 
              class="px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:border-rose-500 transition-colors"
            />
          </div>

          <div class="flex items-center space-x-2">
            <label for="filtro-he-hasta" class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Fecha Final:</label>
            <input 
              type="date" 
              id="filtro-he-hasta" 
          ...
          <button id="btn-limpiar-filtro-he" class="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors active:scale-95 cursor-pointer">
            Todos los Registros
          </button>
        </div>
      ...
      <div class="glass-panel rounded-2xl overflow-hidden border border-slate-200/90 dark:border-slate-800/80 shadow-xl">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead class="bg-slate-50 dark:bg-slate-950/80 text-[11px] uppercase font-semibold text-slate-500 dark:text-slate-400 tracking-wider border-b border-slate-200 dark:border-slate-800">
              <tr>
                <th scope="col" class="py-3.5 px-4 font-mono">Fecha</th>
                <th scope="col" class="py-3.5 px-4 font-mono text-amber-600 dark:text-amber-400">Tramo Inicio HE</th>
                <th scope="col" class="py-3.5 px-4 font-mono text-amber-600 dark:text-amber-400">Tramo Fin HE</th>
                ...
                <th scope="col" class="py-3.5 px-4 text-right font-mono text-rose-600 dark:text-rose-400">Horas Extras a Liquidar</th>
              </tr>
            </thead>
            <tbody id="lista-he-body" class="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white/70 dark:bg-slate-900/30">
        ...
        <tr>
          <td colspan="7" class="py-12 text-center text-slate-400">
            <div class="flex flex-col items-center justify-center space-y-2.5">
              ...
              <p class="text-xs font-medium text-slate-700 dark:text-slate-300">No hay horas extras computadas en este período</p>
        ...
        return `
          <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
            <td class="py-3 px-4 font-mono font-bold text-slate-900 dark:text-white whitespace-nowrap">
              <span class="px-2.5 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-mono">
                ${r.fechaCompleta}
              </span>
            </td>
            
            <!-- Marcador de Inicio tipo Línea de Tiempo -->
            <td class="py-3 px-4 font-mono text-amber-600 dark:text-amber-400 text-xs whitespace-nowrap">
              <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-amber-500/10 border border-amber-500/20">
                <span class="w-1.5 h-1.5 rounded-full bg-amber-500 inline-block mr-1.5 animate-pulse"></span>
            ...
            <td class="py-3 px-4 font-mono text-cyan-700 dark:text-cyan-400 text-xs whitespace-nowrap">
            ...
            <td class="py-3 px-4 font-medium text-slate-800 dark:text-slate-200">
              ${r.descripcion}
              ${r.codigo2 ? `<span class="ml-1 text-xs text-slate-400 dark:text-slate-500 font-mono">(${r.codigo2})</span>` : ''}
            </td>
            <td class="py-3 px-4 text-center font-mono text-slate-600 dark:text-slate-400 whitespace-nowrap">
              ${r.cantidad} u.
            </td>
            <td class="py-3 px-4 text-right font-mono font-bold text-rose-600 dark:text-rose-400 whitespace-nowrap">
              <span class="px-3 py-1 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 shadow-sm text-xs font-bold font-mono">
