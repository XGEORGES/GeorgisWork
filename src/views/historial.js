...
/**
 * Renderiza la interfaz completa de la Pantalla 4 (Historial de Trabajos Terminados - Estilo Syntrix)
 */
    ...
    <div class="space-y-6 pb-16">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <button id="btn-limpiar-filtros-historial" class="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors flex items-center space-x-1.5 active:scale-95 cursor-pointer">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          <div class="p-3 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
            <i data-lucide="check-circle-2" class="w-6 h-6"></i>
          </div>
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Trabajos Concluidos</div>
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Piezas Fabricadas</div>
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Tiempo Total Neto</div>
        ...
        <div class="glass-card p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 shadow-md flex items-center space-x-4 bg-white/90 dark:bg-[#111827]/75">
          ...
          <div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Promedio / Pieza</div>
      ...
      <div class="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 space-y-4 bg-white/90 dark:bg-[#111827]/75 shadow-md">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
          
          <!-- Búsqueda por texto -->
          <div class="md:col-span-4">
            <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Búsqueda rápida</label>
            <div class="relative">
              ...
              <input 
                ...
                value="${filtroTexto}"
                class="w-full pl-8 pr-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-colors"
              />
            </div>
          </div>
          ...
          <div class="md:col-span-3">
            <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Material</label>
            <select 
              id="filtro-historial-material" 
              class="w-full px-2.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:border-cyan-500 transition-colors"
            >
          ...
          <div class="md:col-span-2">
            <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Fecha Desde</label>
            <input 
              type="date" 
          ...
          <!-- Rango: Fecha Hasta -->
          <div class="md:col-span-2">
            <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Fecha Hasta</label>
            <input 
              type="date" 
          ...
          <div class="md:col-span-1 flex items-center justify-end pb-2">
            <span class="text-xs font-mono text-slate-500 dark:text-slate-400" id="contador-historial">0</span>
          </div>

        </div>
      </div>

      <!-- Tabla de Trabajos Terminados (Estilo Syntrix) -->
          ...
          <table class="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead class="bg-slate-50 dark:bg-slate-950/80 text-xs uppercase font-semibold text-slate-500 dark:text-slate-400 tracking-wider border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th scope="col" class="py-3.5 px-4 font-mono text-emerald-600 dark:text-emerald-400">Tiempo Total Fabricado</th>
            ...
            <tbody id="lista-historial-body" class="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white/70 dark:bg-slate-900/30">
        ...
        <tr>
          <td colspan="9" class="py-12 text-center text-slate-400">
            <div class="flex flex-col items-center justify-center space-y-2.5">
              ...
              <p class="text-xs font-medium text-slate-700 dark:text-slate-300">No hay registros en el historial con los filtros aplicados</p>
        ...
        return `
          <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
            ...
            <td class="py-3 px-4 font-mono text-slate-500 dark:text-slate-400 text-xs whitespace-nowrap">
              ${t.codigo2 ? `<span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-mono">${t.codigo2}</span>` : '<span class="text-slate-400 dark:text-slate-600">-</span>'}
            </td>
            ...
            <td class="py-3 px-4 text-center font-mono font-bold text-slate-900 dark:text-white whitespace-nowrap">
              ${t.cantidad} u.
            </td>
            <td class="py-3 px-4 font-mono font-bold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
