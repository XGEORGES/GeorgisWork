...
/**
 * Renderiza el modal de Configuración, Respaldos y Diagnóstico DB (Estilo Syntrix / Dark Telemetry)
 */
export function renderModalConfig() {
  return `
    <div id="settings-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div class="bg-slate-900/90 backdrop-blur-2xl border border-slate-800/90 text-slate-100 w-full max-w-xl rounded-2xl shadow-2xl shadow-black/80 p-6 sm:p-7 relative max-h-[90vh] overflow-y-auto transition-colors">
        
        <!-- Botón de Cierre Superior (X) -->
        <button id="btn-close-settings" class="absolute top-4 right-4 text-slate-400 hover:text-white p-1.5 rounded-xl hover:bg-slate-800/60 transition-colors cursor-pointer">
          <i data-lucide="x" class="w-5 h-5"></i>
        </button>
        
        <!-- Encabezado Modal -->
        <div class="flex items-center space-x-3.5 mb-6">
          <div class="p-3 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 shadow-md shadow-cyan-500/10">
            <i data-lucide="settings" class="w-6 h-6"></i>
          </div>
          <div>
            <h3 class="text-xl font-bold text-white tracking-tight">Configuración del Sistema</h3>
            <p class="text-xs text-slate-400">Preferencias de entorno, copias de seguridad y diagnóstico de base de datos</p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="p-4 sm:p-5 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-3 shadow-inner">
            <div class="flex items-center justify-between">
              <div class="space-y-0.5">
                ...
                <p class="text-xs text-slate-400">Personaliza la visualización de la interfaz según las condiciones de iluminación.</p>
              </div>
            </div>

            <!-- Segmented Switch Moderno -->
            <div class="grid grid-cols-2 gap-1.5 p-1 rounded-xl bg-slate-900/80 border border-slate-800">
              <button 
                id="theme-opt-dark" 
                type="button"
          ...
          <div class="p-4 sm:p-5 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-3.5 shadow-inner">
            <div class="space-y-1">
              <div class="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              ...
              <button 
                ...
                class="py-2.5 px-4 rounded-xl text-xs font-bold bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 flex items-center justify-center space-x-2 transition-all shadow-sm active:scale-95 cursor-pointer"
              >
              ...
              <!-- Importar -->
              <label 
                class="py-2.5 px-4 rounded-xl text-xs font-bold bg-slate-800/80 hover:bg-slate-800 text-slate-200 border border-slate-700 flex items-center justify-center space-x-2 cursor-pointer transition-all shadow-sm active:scale-95"
              >
          ...
          <div class="p-4 sm:p-5 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-3.5 shadow-inner">
            <div class="flex items-center justify-between">
              ...
              <span id="db-status-pill" class="text-xs px-2.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 font-mono border border-emerald-500/30">Conectada</span>
            </div>

            <!-- Detalles del esquema y contadores -->
            <div class="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2">
              <div class="flex items-center justify-between text-xs font-mono">
                <span class="text-slate-400">Motor de Datos:</span>
                <span class="text-slate-200 font-semibold">IndexedDB (Dexie.js v1)</span>
              </div>
              <p id="db-status-details" class="text-[11px] text-slate-400 font-mono break-all leading-relaxed">
            ...
            <div class="flex items-center justify-between pt-1">
              <span class="text-xs text-slate-400">¿Deseas probar la escritura y persistencia local?</span>
              <button 
                id="btn-test-db" 
                type="button"
                class="px-3.5 py-2 rounded-xl text-xs font-semibold bg-cyan-600 hover:bg-cyan-500 text-white shadow-sm transition-all flex items-center space-x-1.5 active:scale-95 cursor-pointer"
              >
          ...
          <!-- Bloque 4: Información de la Aplicación & Soporte -->
          <div class="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800/80 text-xs text-slate-400 space-y-1.5 font-mono">
            <div class="flex justify-between">
              <span>Aplicación:</span>
              <span class="text-slate-200 font-semibold">GeorgisWork CNC v1.0</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between items-center">
              <span>Soporte:</span>
              <a href="mailto:j.huamayalli@gmail.com" class="text-cyan-400 font-semibold hover:underline transition-colors">j.huamayalli@gmail.com</a>
            </div>
          </div>
        ...
        <div class="mt-6 pt-4 border-t border-slate-800 flex justify-end">
          <button id="btn-close-settings-footer" class="px-5 py-2.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-100 transition-all active:scale-95 shadow-sm cursor-pointer">
            Cerrar
          </button>
        </div>
...
export function updateThemeSegmentedUI() {
  ...
  const activeLightClass = 'bg-white text-slate-900 shadow-sm border border-slate-300 font-bold';
  const inactiveClass = 'text-slate-400 hover:text-white border border-transparent font-medium';
